    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">All Withdraw Request</h4>
                        ...
                        <table class="table table-bordered  mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th data-sort="date">Raised By <span class="sort-arrow">&uarr;</span></th>
                                    <th data-sort="string">Amount<span class="sort-arrow">&uarr;</span></th>
                                    <th data-sort="string">Payment Mode<span class="sort-arrow">&uarr;</span></th>
                            ...
                            <tbody id="appendleads">
                                ...
                                @if(!empty($withdraw))
                                @foreach($withdraw as $index => $list)
                                <tr id="lead-row-{{ $list->id }}">
                                    <td>{{ ++ $index }}</td>
                                    <td>{{ $list->user->name ??  '' }}</td>
                                    <td>{{ $list->amount ?? '' }}</td>
                                    <td>{{ $list->payment_mode2 ?? '' }}</td>
                                    <td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" enctype="multipart/form-data" id="updateStatusLead2">
...
<script src="{{ asset('admin/js/ajax/withdraw.js') }}"></script>
...
<script>
    ...
    function updateLeadStatus(id) {
        $.ajax({
            url: "{{ url('admin/withdraw/updatestatus')}}" + "/" + id,
            type: 'get',
            cache: true,
            contentType: false,
            processData: false,
