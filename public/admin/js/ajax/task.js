...
function submitForm(formData) {
    ...
    if ($("#pagename").val() === "next") {
        location =
            window.location.origin + "/admin/leads/edit/" + $("#next").val();
    }
        ...
        success: function (res) {
            ...
            if (res.code == 200) {
                ...
                if (res.code == 215) {
                    ...
                    $("#totalcount").html(`Total: ${res.totalcount} Jobs`);
                } else {
                    if (res.code == 216) {
                        $("#appendleads").html(res.data);
                        $("#totalcount").addClass("col-md-4 alert alert-info");
                        $("#totalcount").html(
                            `Total Sales:  ₹ ${res.totalcount} `
                        );
                    } else {
...
function confirmDelete(id, type) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "/admin/task/delete/" + id, // Update the URL to your delete route
                type: "POST",
                data: {
